...
1. **Define the Global Lock Topology**: Declare the absolute static hierarchy of macro-level locks in the system (`macro_00_global_topology.md`). Determine who holds what lock and in what sequence to guarantee cycle-freedom on a macro level (e.g., `ExfatFs` global allocator lock vs. `ExfatInode` local lock).
2. **Identify Macro-Owners**: Establish the large structural domains (e.g., `ExfatFs`, `ExfatInode`) that will eventually own the Meso-Components.
