...
<!-- List the concrete durable exFAT structures or state machines that act as stable physical truths. -->
...
|---|---|---|---|
| e.g. `StreamExtension` | `ExfatInode(file)` | `ExfatFs` for global coordination notes | File stream state is projected through the file inode runtime authority |
...
|---|---|---|---|---|---|
| e.g. `file_content_mutation` | `ExfatInode(file)` | `write` / `truncate` / `extend` | `StreamExtension`, `AllocationBitmap`, `FAT`, file entry-set metadata | `InodeRwLock(Write)` + lower-level allocator locks only | These features share one content-mutation sequencing and failure domain |
