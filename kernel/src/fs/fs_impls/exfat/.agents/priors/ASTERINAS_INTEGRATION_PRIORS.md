...
Do **NOT** use the flawed legacy Asterinas exFAT implementation as a reference. Everything written in this dictionary is derived from the generic VFS (`kernel/src/fs/vfs`), the OSTD locks (`ostd::sync`), and stable block device layers.
