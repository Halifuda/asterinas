...
//! methods (`store_lower_id` / `read_lower_id`) bridge the record to the
...
pub(in crate::fs::fs_impls::overlayfs) struct LowerIdRecord {
    ...
    /// `st_dev` (`OverlayLayer::container_dev_id`, meso-01 published),
    /// replacing the mount-local `fsid` ordinal (pre-wave5 A2). Stable across
    /// mounts/layer-stack changes; the per-mount ordinal for the xino mask is
    /// derived at read time by matching the `(device, lower-root-ino)` pair
    /// against the immutable current lower snapshot.
...
impl LowerIdRecord {
    ...
    /// constructible this wave. The per-mount `fsid` ordinal is deliberately
    /// NOT persisted: it is derived at read time from the device/root pair.
    /// `Ok(None)` is reserved for the absent export-FH seam (unreachable),
    ...
    /// this wave — the record is a native identity triplet, not an
...
impl OverlayFs {
    ...
    /// Consumed by `IdentityPolicy` via `project_inode` and by meso-04's
    /// re-projection.
