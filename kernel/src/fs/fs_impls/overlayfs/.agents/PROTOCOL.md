...
3. **Subagent Instantiation & Context Policy (amended 2026-08-08, platform-mechanism verified)**: The main agent selects the model and reasoning effort for each delegated Architect, Designer, Creator, Checker, Reviewer, or lightweight triage task according to cost, risk, and required judgment. **Delivery contract**: on this platform the spawn payload, the NEW_TASK header, and followup/send messages are NOT readable by the subagent model; the only readable content channel is the latest user turn at spawn time. Every delegation therefore goes through a **User Dispatch Turn** prepared by the main agent from `protocol/templates/user_dispatch_turn_TEMPLATE.md` and posted by the user, followed immediately by `spawn_agent(task_name=<task_id>, fork_turns="1")`. **Fork policy**: `fork_turns="all"` is forbidden (leaks main-thread identity and context); `fork_turns="none"` is unusable (no readable content, no skill catalog); `fork_turns="1"` is the only allowed value, and only while the latest user turn is the dispatch turn — the user must not post other messages between dispatch and spawn. **Continuation**: followup_task/send_message content is unreadable, so repair, rerun, and continuation rounds are new User Dispatch Turns carrying the Continuation / Parent Task pointer (§1.1); long-lived subagent loops are abolished. **Identity**: subagents cannot read their own task name from a message header; identity is anchored by the dispatch turn wording and cross-checked via `list_agents` (the running non-root agent whose path matches the dispatched task_id); subagents MUST NOT spawn or message other agents even though the tools are present. **Context reality**: UI/thread inspection is not evidence of what the subagent model consumed; acceptance is based solely on artifacts the subagent produced. Results are protocol-valid only when the dispatch turn and the packet name the task ID, role, task kind, risk, scope, write-set, and expected artifact authority clearly enough for the selected agent class.
