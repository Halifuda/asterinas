...
5. **Checker Synchronization & Integration Separation (amended 2026-08-03, user-confirmed — permanent)**: per-pass Creator-synced Checker passes are **replaced** by (a) a wave-level static Reviewer gate (aster-code-review diff-mode, 3-persona fan-out, run after each implementation Wave with a long-lived repair loop) and (b) a **single meso-integration xfstests Checker** (compile + full kernel build + overlay xfstests suite per the Designer validation contracts). Creator passes are command-free and receive **no per-pass compile preflight**; the integration Checker owns the first compile. The meso-integration Checker reports mapped/observed/not-run coverage and is never folded into per-pass slices; a selected upstream test may cover several related micro-features. A validation task may contain multiple isolated runs under that unchanged scope, including compile preflight, rerun, or suffix runs.
