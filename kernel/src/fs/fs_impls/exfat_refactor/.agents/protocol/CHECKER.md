...
18. Treat broad module-like filters as insufficient evidence unless the artifact also explains why the filter cannot silently miss or over-match the intended coverage.
    Do not rely on prefix fragments such as `charset_`: `cargo osdk test` may treat them as non-matching filters and still finish with `0 passed; 0 failed`. Prefer complete test names or complete, source-backed test-path suffixes.
