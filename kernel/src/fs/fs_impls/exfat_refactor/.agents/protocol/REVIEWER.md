...
9. **User-Named Surface Closure Is Mandatory**: If the packet or problem note names concrete symbols, helper families, file-local `#[cfg(ktest)] mod tests`, or test-support paths, you MUST disposition each one explicitly. `Predates this pass`, `already listed by Creator`, or `looks fine` is not an exemption.
...
12. **Final Test-Code Quality Gate**: Checker may record touched test surfaces and obvious concerns, but Reviewer owns the final static judgment for packeted test code. Decide whether helper placement, support hierarchy, naming, file splits, and any `test_support/` rename or migration are acceptable; reject back to Creator cleanup when they are not.
13. **Test-Support Topology Check**: When `#[cfg(ktest)]` support grows beyond a tiny local seam, ensure it lives under a dedicated test-support or successor test-only hierarchy split by concern instead of a flat catch-all sibling file.
