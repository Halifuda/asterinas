...
after the June 2026 exFAT refactor baseline split.
...
- claim an above-`exfat_refactor` owner unless the logs show the smallest
  observed boundary outside the exFAT refactor implementation.
...
- `ABOVE_EXFAT_CONFIRMED`: the failure reproduces and the smallest observed
  failing boundary is outside `kernel/src/fs/fs_impls/exfat_refactor/`.
- `EXFAT_CANDIDATE`: the failure reproduces and the smallest observed failing
  boundary is inside or immediately entered from `exfat_refactor`.
...
- proof that `FSTYP=exfat_refactor`;
...
- why that boundary is above `exfat_refactor`;
