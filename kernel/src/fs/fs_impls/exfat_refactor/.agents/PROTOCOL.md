...
14. Filtered verification commands must prove that they targeted the intended tests. A green exit status alone is insufficient evidence when `cargo osdk test <filter>` is used. The task packet and checker artifact must record either:
    ...
    In particular, plain prefix fragments such as `charset_` are not trustworthy evidence for `cargo osdk test`: the runner may legally report `0 passed; 0 failed` when no full test-path suffix matches. Prefer complete test names or complete, source-justified path suffixes.
