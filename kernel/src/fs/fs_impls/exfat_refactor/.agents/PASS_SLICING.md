...
**Decision:** Architect and Designer artifacts are accepted for the baseline `meso_04` surface, and the follow-up architecture repair for single-entry / published-entry-set field mutation is accepted. This meso now includes one planned post-integration substrate pass for validated field republish and slot-aligned cleanup support in addition to the earlier owner-local skeleton and user-visible create / remove / rename passes.
