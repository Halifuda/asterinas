...
- In scope:
  ...
  - State how the filesystem-owned directory-stream bridge, canonicalization, and child-handle publication interact without inventing a new concurrency owner.
...
- Rule 1:
  - Each `lookup` or `readdir_at` call owns its local record-stream walk; the filesystem-owned directory-stream bridge may create a fresh `DirectoryEngine`, but no call should depend on a shared mutable `DirectoryEngine` instance surviving across calls.
- Rule 2:
  - Name folding and hashing may read filesystem-owned upcase state, but they must not hold that state in a way that overlaps with blocking directory I/O or with the child-publication boundary.
- Rule 3:
  - Child publication or reuse for a matched lookup may enter the filesystem-owned opened-inode boundary only after the directory record match has been identified and the trusted record-location facts required by `InodeKey` have been derived.
  - Those trusted facts are the parent inode identity, the directory byte offset of the record’s primary file dentry, and the primary-entry ordinal within that record.
- Rule 4:
...
- Lookup:
  ...
  - Repeating `lookup` must continue to derive the same `InodeKey` from the same trusted record-location facts, even if the file record is revisited through a fresh `DirectoryEngine`.
- Readdir:
...
- Do not let the directory-stream bridge leak raw `ExfatFs` fields into `inode.rs`.
...
- A directory-record projection that carries trusted location facts is acceptable if those facts remain owner-internal to the read-only directory path.
...
- Reviewers should confirm that the two filesystem-owned bridges remain explicit: one to start a read-only directory stream and one to publish or reuse the canonical child handle.
