...
- In scope:
  ...
  - Consume `DirectoryEngine` as the shared read-only record stream and `UpcaseTable` as the canonical name-folding and name-hash service.
  - Consume `ExfatFs`-owned opened-inode reuse when lookup resolves a child inode.
...
- Out of scope:
  ...
  - Any VFS-facing owner other than `ExfatInode`, including a lookup service, scanner shell, or mutation shell.
...
- Actions:
  ...
  - On a match, resolve the child inode through the filesystem-owned opened-inode reuse boundary so repeated lookup returns the canonical child handle.
...
- Preconditions:
  - The inode is a directory.
- Actions:
  - Drive a read-only directory scan from the current inode’s directory chain using `DirectoryEngine`.
...
- `DirectoryEngine` remains the only record-stream owner and is consumed read-only.
...
- Lock ordering:
  ...
  - If lookup needs canonical child publication, acquire the filesystem-owned publication boundary only around lookup/reuse/insert of the resolved child handle.
...
- Required implementation obligations:
  ...
  - Use `DirectoryEngine` as the record-stream source and `UpcaseTable` as the canonicalization source.
  - Reuse child handles through `ExfatFs` rather than introducing inode-local cache ownership.
...
- Required checker-owned tests:
  - A lookup regression that confirms case-equivalent names resolve through the installed `UpcaseTable` behavior and reuse one canonical child handle.
  - A lookup-miss regression that confirms absent names fail without mutating owner state.
  - A readdir regression that confirms validated file records are emitted in stable order and system entries are not exposed as user-visible children.
...
- Creator slices should be treated as shared-file work in `inode.rs`, not as fake file-parallel lanes.
