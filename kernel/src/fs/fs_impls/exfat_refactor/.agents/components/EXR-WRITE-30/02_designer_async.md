...
- Title: `ExfatInode` write-side serialization and dirty-state visibility
- Status: `Specified`
- Author: designer
- Date: `2026-04-12`
...
- In scope:
  ...
  - State how committed allocation results and inode-local page-cache state are sequenced without inventing a background writer.
...
- Locality of state:
  - Any mutation guard or temporary write context must remain inode-local; it must not become a filesystem-global coordinator.
...
- One inode-local mutation guard is sufficient.
