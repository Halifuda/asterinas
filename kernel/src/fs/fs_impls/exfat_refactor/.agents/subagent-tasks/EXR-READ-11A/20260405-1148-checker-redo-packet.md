...
- Packet ID: `EXR-READ-11A-CHECK-20260405-1148`
- Packet file: `kernel/src/fs/fs_impls/exfat_refactor/.agents/subagent-tasks/EXR-READ-11A/20260405-1148-checker-redo-packet.md`
- Supersedes: none
- Role: checker
- Component: `EXR-READ-11A`
- Phase: serial checker redo
- Authorizing main agent: main-agent
- Date: 2026-04-05 11:48 CST

## Goal

- Re-run the checker pass for `EXR-READ-11A` after the delegated creator pass. Use the exact local ktest names, prove the filter hit coverage, write or adjust checker-owned ktests only if needed, and record the result in `13_checker_serial_retry.md`.
...
- production sections of `inode.rs`, `fat.rs`, and `mod.rs`
...
- Local focus:
  ...
  - directory rejection at the read-view boundary
  - no buffered-read or page-cache ownership drift
...
- In scope:
  ...
  - boundary validation against the latest delegated creator result
- Out of scope:
  - reviewer-level cleanup

## Prior Delivery Notes

- `read.rs` is writable only because the checker owns local ktests there. Do not alter production code in the module unless the packet is explicitly revised.
...
- Do not write reviewer or task-board artifacts.
