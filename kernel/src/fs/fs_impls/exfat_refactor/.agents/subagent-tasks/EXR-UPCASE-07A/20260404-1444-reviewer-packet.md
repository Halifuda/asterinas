...
- Packet ID: `EXR-UPCASE-07A-REVIEW-20260404-1444`
- Packet file: `kernel/src/fs/fs_impls/exfat_refactor/.agents/subagent-tasks/EXR-UPCASE-07A/20260404-1444-reviewer-packet.md`
- Supersedes: none
- Role: reviewer
...
- Perform bounded static review of the `EXR-UPCASE-07A` implementation after creator and checker completion. Focus on boundary hygiene, visibility discipline, invariant expression, and keeping the component strictly narrower than case folding or name hashing. Do not run commands.
...
- Local focus:
  - `upcase_table.rs` must remain loader-only
  - no fallback policy, case folding, or name hashing
...
- Keep review bounded to `upcase_table.rs`.
...
- The loader entry point and read-only table surface are already justified by the accepted designer spec.
...
- Known conflicts:
  - any lane writing `upcase_table.rs` or the reviewer artifact
