...
**Component/Task Group:** `meso_04_directory_entry_mutation`
**Parent Meso-Component:** `meso_04_directory_entry_mutation`
...
- **Output Destination:** `kernel/src/fs/fs_impls/exfat_refactor/.agents/components/meso_04_directory_entry_mutation/meso_04_directory_entry_mutation_architecture.md`
...
- Map only `meso_04_directory_entry_mutation` from the accepted macro topology.
- Pull all relevant micro-features for `create`, `mkdir`, `unlink`, `rmdir`, `rename`, entry-set mutation, emptiness gates, and mutation-side handling of unrecognized entries.
- Treat read-side lookup / identity rules as already mapped by `meso_03_directory_lookup_and_identity`; do not remap those rows except as structural interactions.
- Treat allocator accounting and volume-state posture as imported neighboring constraints from `meso_02` and `meso_08`.
