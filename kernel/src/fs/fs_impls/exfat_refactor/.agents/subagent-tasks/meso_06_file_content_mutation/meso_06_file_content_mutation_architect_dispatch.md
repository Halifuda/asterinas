...
**Component/Task Group:** `meso_06_file_content_mutation`
**Parent Meso-Component:** `meso_06_file_content_mutation`
...
- **Output Destination:** `kernel/src/fs/fs_impls/exfat_refactor/.agents/components/meso_06_file_content_mutation/meso_06_file_content_mutation_architecture.md`
...
- Map only `meso_06_file_content_mutation` from the accepted macro topology.
- Pull all relevant micro-features for `write_at`, `resize`, append growth, truncate/shrink, zero-fill, `NoFatChain` transitions, timestamp updates, and fallocate-family refusal.
- Treat allocator accounting / recount as imported neighboring constraints from `meso_02`; treat global dirty/anomaly posture as imported neighboring constraints from `meso_08`.
- Keep file-scoped `fsync` outside this meso except as an external structural interaction with `meso_07_file_sync_and_persistence`.
