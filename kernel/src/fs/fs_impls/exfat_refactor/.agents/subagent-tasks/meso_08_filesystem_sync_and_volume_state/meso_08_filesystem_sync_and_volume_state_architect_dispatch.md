...
- Map only `meso_08_filesystem_sync_and_volume_state` from the accepted macro topology.
- Pull all relevant micro-features from the authorized inventory for filesystem-wide `sync`, clean unmount/quiesce, mount-time anomaly interpretation, and volume-state transitions.
- Keep mount-time anomaly posture import as an upstream interaction with `meso_01_mount_volume_state`; this meso owns later filesystem-wide transitions and quiesce boundaries.
- Keep file-scoped `fsync` and ordinary-file flush semantics outside this meso except as external structural interactions with `meso_07_file_sync_and_persistence`.
