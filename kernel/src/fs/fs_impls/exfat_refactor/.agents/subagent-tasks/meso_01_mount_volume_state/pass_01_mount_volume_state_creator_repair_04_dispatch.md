...
- This is a follow-up Creator repair for the latest Checker repair batch in `.agents/components/meso_01_mount_volume_state/pass_01_mount_volume_state_checker.md`.
- Apply only the Checker's actionable repair batch for the baseline `InvalidOnDiskLayout` failure. Do not reinterpret unrelated tests or widen the pass.
