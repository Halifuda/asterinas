...
- This redispatch supersedes `.agents/subagent-tasks/meso_01_mount_volume_state/pass_01_mount_volume_state_creator_repair_01_dispatch.md`, which is withdrawn.
- Implement this pass as one complete `meso_01_mount_volume_state` Creator slice covering exactly the micro-features listed above; do not split the pass further.
- Build fresh refactor-owned code under `kernel/src/fs/fs_impls/exfat_refactor/`. Do not reference or reuse legacy `kernel/src/fs/fs_impls/exfat/` implementation code.
