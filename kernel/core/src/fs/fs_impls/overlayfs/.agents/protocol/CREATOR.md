...
8. **Repository-Entry Compile and Clippy Preflight**: Creator remains a command-free role for QEMU, NixOS, xfstests, and other runtime validation, but an implementation pass is compile-ready by default only after a successful target-specific `cargo check`. Before returning the Creator report, run inside the verified `codex-asterinas-dev` container:
   - **Default compile gate:** `docker exec -w /root/asterinas codex-asterinas-dev bash -lc 'cd /root/asterinas/kernel && cargo check -p aster-kernel --target x86_64-unknown-none'`
   - **Clippy when lint preflight is packeted or needed to diagnose compile quality:** `docker exec -w /root/asterinas codex-asterinas-dev bash -lc './tools/clippy_check.sh workspace'`
   Use `docker exec -w /root/asterinas codex-asterinas-dev make check` for full repository static parity or `docker exec -w /root/asterinas codex-asterinas-dev make kernel` when the packet explicitly requires the full kernel build. If the default compile gate cannot be run because the packet explicitly withholds compile capability, do not claim the result is compile-ready; record the omission and exact reason in the Creator report. Never use a host-side or repository-root `cargo check` with an implicit target, omit the exact command and toolchain from the Creator report, or run runtime validation; the Checker owns the execution lock and runtime validation loop. Do not use a short command timeout that can interrupt Rust, dependency, or toolchain downloads: allow at least 10 minutes for each first-run or toolchain-changing command, and extend it when downloads are still active. Record the exact command, toolchain, timeout, and output.
...
10. **No Legacy filesystem Oracle**: Do not treat the existing `kernel/core/src/fs/fs_impls/overlayfs/` implementation as a design oracle, scaffold source, or structure template for refactor work. Implement from the accepted Architect/Designer artifacts plus stable Asterinas kernel interfaces only. If a packet points you at legacy `exfat` implementation files, stop and report the packet violation instead of mining that code.
11. **No Architectural Revisions**: If the Designer's spec is fundamentally unimplementable in Rust (e.g., contradictory lifetimes across the `[GUARANTEE]` boundary or a frozen signature), document it in your report and return the task. Do not silently change the Designer's frozen signatures, widen the covered-micro scope, or rewrite the lock topology.
