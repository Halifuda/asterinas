...
5. **Repository-Entry Compile and Clippy Commands**: Run compile and lint validation only inside the verified container and use these exact commands:
   - **Target-specific Rust compile smoke:** `docker exec -w /root/asterinas codex-asterinas-dev bash -lc 'cd /root/asterinas/kernel && cargo check -p aster-kernel --target x86_64-unknown-none'`
   - **Workspace Clippy:** `docker exec -w /root/asterinas codex-asterinas-dev bash -lc './tools/clippy_check.sh workspace'`
   - **Repository static/build parity:** `docker exec -w /root/asterinas codex-asterinas-dev make check`
   - **Full kernel build:** `docker exec -w /root/asterinas codex-asterinas-dev make kernel`
   Use the target-specific `cargo check` when the packet requests a minimal compile smoke; use the Clippy command when isolating the Rust compile/lint stage or when the packet requires lint evidence. Do not substitute a host-side or repository-root `cargo check`, an implicit target, or a hand-selected feature set, because that can bypass the pinned toolchain, OSDK, initramfs, and target configuration. Record the exact command, toolchain, timeout, and output. Never use a short command timeout that can interrupt Rust, dependency, or toolchain downloads: allow at least 10 minutes for each first-run or toolchain-changing command, and extend it when downloads are still active. A passing compile smoke does not imply a passing `make check`: formatting, whitespace, Nix, C, and typo gates must be reported separately.
6. **Runtime Makefile Command**: For kernel execution, use the CI/Makefile form `docker exec -w /root/asterinas codex-asterinas-dev make run_kernel AUTO_TEST=<mode>` or the packet-authorized `$ovfs-checker` wrapper. Preserve the exact command and runtime evidence; do not invoke an ad hoc `cargo osdk run` command with locally chosen configuration.
