...
This file is the main-agent-owned scheduler protocol for the `kernel/core/src/fs/fs_impls/overlayfs` workspace.
...
- **Checker Pass**: A validation slice. For implementation work it MUST mirror the Creator Pass exactly; meso-level integration validation is scheduled as an independent Checker-only pass with its own covered-micro set. Behavioral validation is xfstests-only (amended 2026-08-31: unit-test execution under the scoped allowance of amended rule 17 is a packeted Checker Validation Run, not a ktest behavioral lane).
...
- **User Dispatch Turn**: the V2 **fallback** task-delivery channel (amended 2026-08-10 — the preferred channel is the Direct Spawn Lane, §1.3): a user-authored message that names the task_id, the packet path, the role rules, and the report contract; the main agent spawns the subagent with `fork_turns="1"` immediately after it (see §1.3). The dispatch turn is a pointer route plus identity anchor, never a copy of the packet.
...
3. **Subagent Instantiation & Context Policy (amended 2026-08-10, platform-mechanism verified; Direct Spawn Lane PREFERRED, V2 lane FALLBACK)**: The main agent selects the model and reasoning effort for each delegated Architect, Designer, Creator, Checker, Reviewer, or lightweight triage task according to cost, risk, and required judgment.

   **Direct Spawn Lane (PREFERRED, platform-verified 2026-08-10).** Spawn the subagent with `fork_context=false` (no fork of parent history — the parent thread's user/assistant content is NOT the subagent's context) and a fully self-contained initial `message` that carries: (a) the role declaration ("你是被派出的协议角色子代理，不是主代理"), (b) the `task_id`, (c) the role-rules path (`protocol/<ROLE>.md` + `PROTOCOL.md`), (d) the packet path (`subagent-tasks/<component-id>/<task_id>_dispatch.md` — the sole normative contract on the shared filesystem), (e) the exact write-set (only the packet-authorized artifact paths; nothing else may be created/moved/deleted), (f) the prohibitions (no spawn/followup/send_message, no build/test/run commands unless the packet grants Checker capability, no `.agents` record edits, no production `.rs` edits, no ktest surface beyond the amended rule 17 allowance), and (g) the report contract (final reply starts with `task_id` and lists artifact paths). No user-posted dispatch turn is required; no `task_name`/`fork_turns` parameters are used on this platform (the spawn returns the agent id and a nickname; the subagent states its own identity in its reply and may cross-check with `list_agents`). Continuation/repair rounds are **new spawns** carrying the Continuation / Parent Task pointer — followup/send messages are not a valid channel.
   Verification evidence (2026-08-10): the Designer task `task_designer_wave7_cache_consistency_20260810` (nickname Planck) was dispatched this way and completed with an exact write-set; pingpong rounds 1 (Carson) and 2 (Russell) both received the initial message and replied correctly without tool use; an earlier fork-based spawn (Galileo, `fork_context=true`) misread the forked parent context and violated the write-set, which is the reason the direct lane is preferred.

   **V2 dispatch lane (FALLBACK, previously verified 2026-08-08).** If the direct initial-message channel is unavailable, or a strict pointer route with zero payload is required, fall back to the **User Dispatch Turn** prepared by the main agent from `protocol/templates/user_dispatch_turn_TEMPLATE.md` and posted by the user, followed immediately by `spawn_agent(task_name=<task_id>, fork_turns="1")`. Under this fallback, the only readable content channel is the latest user turn at spawn time; the spawn payload and followup/send messages are NOT readable by the subagent model.

   **Fork policy (applies to the fallback lane; the direct lane does not fork):** `fork_turns="all"` is forbidden (leaks main-thread identity and context); `fork_turns="none"` is unusable (no readable content, no skill catalog); `fork_turns="1"` is the only allowed value, and only while the latest user turn is the dispatch turn — the user must not post other messages between dispatch and spawn. **Continuation**: repair, rerun, and continuation rounds are new dispatch turns / new spawns carrying the Continuation / Parent Task pointer (§1.1); long-lived subagent loops are abolished. **Identity**: subagents cannot read their own task name from a message header; with the direct lane the initial message anchors identity and the subagent states it in its reply (cross-check with `list_agents` when available); subagents MUST NOT spawn or message other agents even though the tools are present. **Context reality**: UI/thread inspection is not evidence of what the subagent model consumed; acceptance is based solely on artifacts the subagent produced. Results are protocol-valid only when the message and/or dispatch turn plus the packet name the task ID, role, task kind, risk, scope, write-set, and expected artifact authority clearly enough for the selected agent class.
4. **Pipeline Gates & Pass Slicing**: No Meso enters implementation before its Architect handoff and applicable Designer artifacts (`_designer_spec.md` and a validation contract) exist. Designer contracts may be revised or substantially rewritten in bounded Meso waves; a later Meso's Designer work need not wait for every other Meso if dependencies and write-sets allow. A revision must preserve the parent Meso, covered Micro set, and accepted static topology; static owner or lock-topology defects require a bounded Architect repair. New Designer validation contracts MUST describe an external validation mapping using xfstests; ktest-based validation is forbidden except as scoped by amended rule 17 (unit-test design for pure-logic surfaces when the packet authorizes it). No packet may create, modify, or grow `#[ktest]`, `#[cfg(ktest)]`, kernel-mode test modules, `test_support/`, memory-disk fixtures, or other ktest harnesses anywhere in the repository beyond the amended rule 17 allowance. The main agent decides every Creator Pass boundary, records it in `PASS_SLICING.md` before or with dispatch, and requires every Creator / Checker / Reviewer packet to name exactly one parent meso-component plus explicit covered micro-features. Architects and Designers stay exhaustive at the Meso level and must not pre-slice implementation passes.
5. **Checker Synchronization & Integration Separation (amended 2026-08-03, user-confirmed — permanent)**: per-pass Creator-synced Checker passes are **replaced** by (a) a wave-level static Reviewer gate (aster-code-review diff-mode, 3-persona fan-out, run after each implementation Wave with a long-lived repair loop; rounds are dispatched per §1.3, each round a fresh User Dispatch Turn — followup continuation is not a valid channel on this platform) and (b) a **single meso-integration xfstests Checker** (compile + full kernel build + overlay xfstests suite per the Designer validation contracts). Creator passes are command-free and receive **no per-pass compile preflight**; the integration Checker owns the first compile. The meso-integration Checker reports mapped/observed/not-run coverage and is never folded into per-pass slices; a selected upstream test may cover several related micro-features. A validation task may contain multiple isolated runs under that unchanged scope, including compile preflight, rerun, or suffix runs.
...
16. **Reviewer Authority & Final Checker**: Reviewer normally runs after Checker; an extra pre-checker structural audit is allowed only when explicitly requested by the user or main agent and does not replace the ordinary post-checker Reviewer gate. **For this overlayfs refactor (amended 2026-08-03, user-confirmed):** the wave-level Reviewer gate runs **before** code completion (per rule 5) — it is the only static gate until the integration Checker; the post-checker Reviewer still applies after the meso-integration gate at acceptance. The main agent may group stabilized passes into a bounded review wave for one parent Meso when the packet enumerates every pass and the exact covered Micro union. Reviewer direct edits are limited to line-level non-functional changes that preserve behavior and topology; structural findings reopen Creator cleanup rather than being rewritten by Reviewer.
17. **Refactor Validation Boundary (amended 2026-08-31, user-confirmed):**
    Behavioral validation of the overlayfs refactor remains xfstests-only. In
    addition, a narrowly scoped **unit-test lane** is now permitted: Rust unit
    tests for self-contained, pure-logic surfaces of the overlayfs
    implementation (e.g., option parsing, name mapping/escaping, encoding) MAY
    be designed and later implemented as `#[ktest]` test modules inside
    `kernel/core/src/fs/fs_impls/overlayfs/`, provided that (a) the targets
    are named by an accepted Designer artifact, (b) the unit tests assert only
    the pure surface's own contract — no mounted-filesystem, VFS, block, or
    other I/O fixtures — and (c) the dispatch packet explicitly authorizes
    them. Executing such unit tests is a packeted Checker Validation Run via
    the Makefile form (`make ktest`). Regression tests live under
    `test/initramfs/src/regression/` and follow the repository testing
    guideline (`book/src/to-contribute/coding-guidelines/for-development/testing.md`).
    No other ktest surface (behavioral kernel-mode suites, `test_support/`,
    memory-disk fixtures) is authorized; xfstests remains the sole lane for
    end-to-end overlayfs behavior validation.
...
- **Designer**: Maps the authoritative design documents and the accepted
  ...
  spec and one external-validation contract containing pass-scoped
  test mappings and integration obligations. Behavioral validation design is
  xfstests-only; ktest unit-test design is allowed only for pure-logic surfaces
  named by the artifact and explicitly authorized by the packet (amended rule
  17, 2026-08-31).
...
2. `Specified` means all Designer artifacts exist, explicitly covering dynamic lock behavior, the meso-level Rust code-form signature design (module layout, struct/enum/carrier/helper signatures per the coding guidelines), the external validation mapping, and meso-level integration obligations; ktest content is allowed only within the amended rule 17 unit-test allowance.
