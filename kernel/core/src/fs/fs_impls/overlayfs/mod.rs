...
//! | copy-up coordinate | The pending publication name that every lower-backed object carries from projection time; its one-time publication retires it permanently. | `OverlayInode.copyup` (`inode/mod.rs`) |
//! | recorded parent | The logical parent recorded when a name is first projected; it is the copy-up publication parent and is repointed only by a cross-directory rename. | `OverlayInode.recorded_parent` (`inode/mod.rs`) |
