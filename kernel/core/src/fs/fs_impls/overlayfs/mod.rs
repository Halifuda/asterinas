...
//! | copy-up | The lower-to-upper promotion of a written object: stage a private workdir temp, then atomically rename it into place, ancestors first. | `OverlayInode::copy_up` (`inode/copyup/mod.rs`) |
//! | publication coordinate | The (publication parent, name) pair a copy-up publishes at, sourced per operation from the overlay dentry (`CopyUpOrigin::Operation`) or, for the dentry-less entries, from the recorded parent plus the visible-source real dentry name (`CopyUpOrigin::Recorded`). | `CopyUpOrigin` (`inode/permission.rs`), `OverlayInode::publication_coordinate` (`inode/copyup/mod.rs`) |
//! | recorded parent | The logical parent recorded when a name is first projected; it is the readdir `..` identity authority and the copy-up publication parent for the dentry-less entries only, and is repointed only by a cross-directory rename. | `OverlayInode.recorded_parent` (`inode/mod.rs`) |
//! | admission | The single permission gate of every request: the local check, copy-up promotion for mutating requests, then the real-handle re-check. | `OverlayInode::check_permission`, `AccessType` (`inode/permission.rs`) |
